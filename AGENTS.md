...
## CONTEXTUAL RULES
- `se.deversity.vibetags.processor.AIGuardrailProcessor.earlyExitAllowed()`: Focus on A step added after the collection walk that reads the collected model must also turn the early exit off here. Avoid Adding such a step without a clause here: a no-op rebuild then skips it with nothing reporting it (#834).
- `se.deversity.vibetags.processor.AIGuardrailProcessor.finishUnchangedBuild()`: Focus on Every diagnostic generateFiles() raises before its fingerprint short-circuit must also be raised here. Avoid Adding one there without a call here: a no-op rebuild that takes the early exit drops it in silence (#859).
...
- **se.deversity.vibetags.processor.AIGuardrailProcessor** (sensitivity: critical): Runs inside every consumer's javac: process() turns a RuntimeException into a WARNING (an ERROR in check mode), and anything escaping that catch fails their build
- **se.deversity.vibetags.processor.internal.GuardrailFileWriter** (sensitivity: high): Invariant 2 lives here: hand-authored content outside VIBETAGS-START/END must never be overwritten or lost
- **se.deversity.vibetags.processor.internal.ModuleSidecar** (sensitivity: high): The .vibetags-mod-* format is read by modules compiled separately, possibly by another processor version; changing it breaks them
...
- **se.deversity.vibetags.processor.internal.WriteCache** (sensitivity: high): A false positive, stale output taken as unchanged, silently leaves generated files wrong
...
- **se.deversity.vibetags.processor.internal.TransitiveManifestReaderLimitsTest**: Security-critical code [Pins the MAX_LOOKUPS cap and the SKIPPED_PREFIXES list of TransitiveManifestReader; relaxing a case to make it pass widens what a dependency may put in front of an agent]. Do not weaken security properties. Flag any change for security review.
