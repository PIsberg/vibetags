...
| 29 | Inquisitor on PRs | 2 | Two lanes, so every PR gets a machine reviewer whenever either has credit: `copilot-review.yml` requests a GitHub Copilot review automatically and then VERIFIES the request landed, because the request API returns success even when GitHub silently drops the reviewer (observed live on PR #417: 2xx from the POST, no review-request event recorded, with both the Actions token and the maintainer's own; the ruleset parameter for automatic Copilot review was likewise silently stripped). Until Copilot code review is actually active for this account, the lane reports SKIPPED on every PR - loudly, per the accepted design. `inquisitor.yml` enforces the committed law whenever `ANTHROPIC_API_KEY` exists. Promoting the Inquisitor to a required check stays a deliberate later step. |
