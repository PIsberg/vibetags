...
[architecture/components.md](architecture/components.md#parsed-diagrams-code-karta). Adding an annotation means
...
- `@AIIgnore` present but no `.cursorignore` / `.qwenignore` / `.aiexclude` exists — orphaned ignore annotation (no warning names a deprecated file, so `.claudeignore` and `.copilotignore` are not suggested, #667, #668)
