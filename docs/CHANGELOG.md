...
- **"Renders as another platform" is now a property, `Platform.rendersAs()` (#764).** `GEMINI.md`
  ...
  on purpose: that is a list of files that get a section, not of wordings. Not done here: the five
  pass-through renderers (Firebase, Cline, Void, Goose, Replit) still exist as classes.
