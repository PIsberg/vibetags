...
Five platforms have both an always-loaded aggregate file and a glob-scoped granular directory: `CLAUDE.md`↔`.claude/rules/`, `.cursorrules`↔`.cursor/rules/`, `.windsurfrules`↔`.windsurf/rules/`, `.github/copilot-instructions.md`↔`.github/instructions/`, `GEMINI.md`↔`.gemini/rules/`. When **both** are opted in, the aggregate renderer collapses to a **scoped-rules index**: only the always-loaded safety buckets stay inline (`@AILocked`, `@AICore`, `@AIPrivacy`, `@AIIgnore`, `@AIAudit`, `@AISecure`), plus one line per element naming the element (`GranularIndexSection`); every other bucket moves to the granular files. The scoped file's name is derived from the element path (every non-alphanumeric character becomes `-`), stated once in the index note rather than repeated per entry; only an element a `.vibetags-roles` config routes onto a shared role file carries an explicit path. The index note for `GEMINI.md` does not say the scoped files load on their own, because Gemini CLI never reads `.gemini/rules/`; it tells the agent to open the file with `read_file` (#669).
