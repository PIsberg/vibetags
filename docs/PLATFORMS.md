...
| `TESTING.md` | No tool reads it by name. Opt-in routing target: when present, a round that compiles test code (source set `test`, `*Test`, `*Tests` or `testFixtures`) renders its guardrails here. A main round writes nothing to it | Markdown, the `AGENTS.md` layout under a two-line preamble |
