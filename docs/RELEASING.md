...
> **Do not bump `load-tests/pom.xml`.** Its `<processor.version>` is pinned deliberately so
> benchmarks can compare across releases, and `scripts/set-version.sh` skips it for that reason.
> This table used to list it as a required bump, which contradicted the script; the pin is
> correct and the row was wrong.
...
> Bumping `vibetags/pom.xml` and `vibetags/build.gradle` is what moves it.

> Tip: `grep -rn "<old-version>" --include="*.xml" --include="*.gradle" --include="*.md" --include="*.java" --exclude-dir=.git --exclude-dir=target --exclude-dir=build --exclude-dir=results --exclude-dir=node_modules --exclude-dir=changelog-assets .` catches every spot. Expected residue after a bump: the previous version's CHANGELOG entry, frozen `load-tests/results/<old>/` baselines, `load-tests/pom.xml` (pinned — see above), and `load-tests/dependency-reduced-pom.xml` (auto-regenerated by maven-shade on next build). The `.git` exclusion skips `PR_BODY.md` / `RELEASE_NOTES.md` scratch files left by earlier releases, which are full of stale versions and must not be edited.

#### Bumping the version with `scripts/set-version.sh`

There is deliberately no parent POM tying `vibetags-annotations`, `vibetags`, and `vibetags-bom`
together (they are independent Maven modules — see `CLAUDE.md`), so the core library version is
hardcoded in each of them. `scripts/set-version.sh` updates all of it in one pass:

```bash
scripts/set-version.sh 1.0.0
```

It rewrites, in place, every occurrence of the current version (read from
`vibetags-bom/pom.xml`'s `<vibetags.version>` property) to the new version across:

- `vibetags-annotations/pom.xml` — `<version>`, plus the Maven/Gradle install snippets in `<description>`
- `vibetags-annotations/build.gradle` — the top-level `version` and the `mavenJava` publication `version`
- `vibetags/pom.xml` — `<version>`, plus the `<dependency>` on `vibetags-annotations`
- `vibetags/build.gradle` — the top-level `version`, the `mavenJava` publication `version`, and the
  `api 'se.deversity.vibetags:vibetags-annotations:...'` dependency line
- `vibetags-bom/pom.xml` — `<version>`, the managed `<vibetags.version>` property, and the install
  snippets in `<description>`

It is a plain POSIX `sh` script (runs under Git Bash on Windows) and is idempotent: it reads the
current version fresh from `vibetags-bom/pom.xml` on every invocation, so running it again with the
same target version is a no-op, and running it with a different target version later picks up
correctly from whatever is on disk.

It deliberately does **not** touch `example/pom.xml`, `example/build.gradle`, `tools/demo/pom.xml`
(these track a *released* version via `vibetags.bom.version` and are bumped separately once a
release exists), or `load-tests/pom.xml` (its `<processor.version>` is intentionally pinned
independently for cross-version benchmark comparisons). You still need to update those,
`README.md`, and `CHANGELOG.md` by hand per the table above.
