...
| `example/pom.xml`, `example/build.gradle`, `example-multimodule/pom.xml`, `example-multimodule-indexed/pom.xml`, `tools/demo/pom.xml` | Standalone on purpose so a user can lift them into their own project; CI builds them against the artifacts this repo just installed, so they track the current version |
