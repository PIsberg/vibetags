...
| `GranularIndexEndToEndTest` | Scoped-rules index: dual opt-in (aggregate + granular) collapses the aggregate to a locked/core/safety summary plus a per-element index; single opt-in stays full; reuse renderers (Cline) stay full while `CLAUDE.local.md` mirrors `CLAUDE.md`; the collapsed `GEMINI.md` prints no heading its full render does not (Gemini's title, locked heading and section wording, #721); every Gemini render (full and collapsed `GEMINI.md`, `gemini_instructions.md`) sets each heading and audit block off by exactly one blank line (#723) |
