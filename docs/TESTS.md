...
| `CaseCollidingGranularStemTest` (`e2e`) | Two elements whose rule filenames differ only in capitalisation, e.g. the package `com.example.payment` and the class `com.example.Payment`. A case-sensitive filesystem holds two files and both guardrails survive; Windows and macOS hold one, and one element's guardrails are gone while the scoped-rules index still names both. Asserts the warning rather than the files on purpose: which file survives is a property of the runner, so a "two files exist" assertion passes on Linux CI precisely where the bug does not bite |
