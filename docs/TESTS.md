...
| `SupersededAncestorRegionTest` | The boundaries of that rule at `readAll` level: an ancestor fully covered by one subproject is deleted, one with an element of its own survives, siblings never supersede each other, coverage runs downward only and transitively, both source sets of the winning region count, a sidecar with no element ids is left alone, and `peekAll` excludes without deleting |
