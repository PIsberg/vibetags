...
| `GranularIndexEndToEndTest` | Scoped-rules index: dual opt-in (aggregate + granular) collapses the aggregate to a locked/core/safety summary plus a per-element index; single opt-in stays full; reuse renderers (Cline) stay full while `CLAUDE.local.md` mirrors `CLAUDE.md`; the collapsed `GEMINI.md` prints no heading its full render does not (Gemini's title, locked heading and section wording, #721) |
...
| `SectionCatalogContractTest` | `SectionCatalog`'s caller-visible behaviours: `header()` null exactly when `isHeaderless()` says so (every platform × key), a registered platform override winning over the shared default, headerless as a per-pair property, and both Gemini platforms resolving the same wording for every key (#721) |
