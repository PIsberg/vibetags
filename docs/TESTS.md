...
| `AggregateBlankLineContractTest` | `llms-full.txt`, `CONVENTIONS.md`, `.github/copilot-instructions.md`, `.junie/guidelines.md` and `.junie/AGENTS.md` set every heading off by exactly one blank line (#725, #726): all 44 annotations populated, bare, and with only optional members unset, plus a nothing-locked model that must print no empty locked heading. Asserts both directions, because a `\n\n\n` check alone passes when a formatter arm closes with a single newline and glues the next heading to its last bullet |
