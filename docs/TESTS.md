...
| `ProjectLifecycleEndToEndTest` | The project's timeline rather than one compile: day zero with nothing opted in (no guardrail file written, and the footprint pinned to VibeTags' own three state files), the steady state of repeated no-change builds (every generated file byte-identical *and* untouched across three builds), a platform opted into months later, and a module deleted or renamed out of a reactor. Three of its tests pin a measured limitation rather than a feature: the fingerprint short-circuit that cannot fire while the round rewrites its own sidecar, the newly opted-in file that carries only the modules that recompiled since, and the deleted module's granular rule files that no later build retires. Each says what it costs and what to assert instead once it is fixed |
