...
remaining ~60 constants render no interpolated value (
...
| `NewPlatformsEndToEndTest` | Windsurf, Zed, Continue, Tabnine, Amazon Q, universal `.ai/rules/` |
...
| `DeprecatedServicesTest` | A build with a deprecated output opted in (#641) gets exactly one compiler warning naming every such file or directory and its replacement, each deprecated output opted in on its own is warned about by name (one table in the test lists every notice, so a row announced in the docs but missing from `DeprecatedServices` fails by key), one `platform.deprecated key= file= replacement=` log event per file, and no warning for a `.clinerules/` directory; the no-opt-in note stops offering deprecated files; every row's key is an opt-in key and its file is the mapped path. Confirmed red against an empty table and again with the call removed from `ServiceRegistry` |
| `RemovedPlatformOutputsTest` (`e2e`) | The four outputs removed in 2.0.0 (#645): a `gemini_instructions.md`, `.cody/config.json`, `.codyignore`, `.supermavenignore` or single `.clinerules` file left on disk is not written and no warning names it; none of the five keys is an opt-in key, a mapped service, a `Platform` or a `DeprecatedServices` row; a `.clinerules` file activates nothing while the directory still activates `cline_granular`; `.aiexclude` is written beside `GEMINI.md`. All 8 cases red before the removal |
...
| `ClineSafetyTierEndToEndTest` | A `.clinerules/`-only project gets `.clinerules/+vibetags-safety.md` with no front matter carrying the safety tier and not the verbose tier; the orphan sweep leaves it alone; hand text outside markers survives; removing the last safety annotation retires its guardrail; no element or role stem can take its name; a leftover single `.clinerules` file gets no safety file and is not written (#645); a reactor merges every module into it; check mode agrees (#648) |
...
| `ClineRulesDirectoryEndToEndTest` | Cline's `.clinerules/*.md` directory form: per-element rules carry `paths:` front matter; a hand-written rule in the directory stays byte-identical; Cline's own file-to-directory conversion, from the file a 1.x build left and 2.0.0 leaves alone, keeps the hand text in `default-rules.md` and sweeps the stale VibeTags block (#642, #645) |
...
|---|---|
| `InitCommandTest` | `vibetags init`: `--list` shows opt-in keys without creating anything, marking `[active]` by the kind of entry each service writes; `--platforms` creates empty opt-in files (directories for `*_granular` keys, parents for nested paths); an existing file is reported active and never truncated; a path held by the wrong kind of entry (a leftover `.clinerules` file when `cline_granular` is requested) is refused, not reported active; the outputs removed in 2.0.0 are neither listed nor accepted (#645); an unknown key rejects the whole request before creating the valid half; bare `init` creates nothing; `--dir` targets another root |
