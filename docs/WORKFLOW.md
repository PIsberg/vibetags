...
- **Gradle granular assertions** (JDK 21 leg, inside the `gradle-multimodule` step). Dual opt-in
  ...
  correctly.
- **Gradle reactor depth**, six steps, JDK 21 leg (issue #443). The Maven reactor had eleven
  ...
  root; and role-based granular grouping across modules, including the issue #365 repro that
  building one subproject must leave the shared role file byte-identical.
