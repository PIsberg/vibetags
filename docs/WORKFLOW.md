...
6d. **Run vibetags init --list Against The Basic Example** (JDK 21 only) — asserts the opt-in platform keys are printed and that `git status` for `examples/basic/` is still clean afterwards. `--list` prints; it must create nothing.
