...
2. The integration and end-to-end tests compile annotated fixture sources in-memory via `ProcessorTestHarness` and run as part of the plain `mvn test` suite; they do not read `example/` (the `-Drun.integration.tests=true` gate that once tied them to it was dropped in 2026-04).
