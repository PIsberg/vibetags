...
This design shipped as specified. The as-built logger-name-suffixing code, the committed `junit-platform.properties`, and the verification test are documented in one place: [docs/architecture/testing.md § Concurrency & Thread-Isolated Logging](../architecture/testing.md#concurrency--thread-isolated-logging).
