...
|---|---|---|
| [`gradle-multimodule/`](gradle-multimodule/) | Subprojects below the root, each with its own `build.gradle` | `-Avibetags.root` at the reactor root. Also the Gradle side's depth fixture: five modules, all 44 annotations, transitive manifests, mirroring, roles, nested output, check mode and all 51 services. |
...
| [`gradle-flat/`](gradle-flat/) | Module beside the root, not below it | `-Avibetags.root` at the directory containing both, or module ids become path hashes that differ per checkout. Names its modules explicitly, as `gradle-composite` does |
...
| [`kotlin-ksp/`](kotlin-ksp/) | Gradle, Kotlin DSL, KSP | The same sources as `kotlin/`, compiled through KSP with `vibetags-ksp` instead of kapt. CI fails unless the generated files equal `kotlin/`'s byte for byte, and unless the build warns about the guardrail kapt drops silently. |
