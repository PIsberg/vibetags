...
<project_guardrails>
  <locked_files>
    ...
    <file path="com.example.multimodule.core.IrGraphFixturesTest">
      <reason>The golden graphs below are the layout engine&#39;s recorded output for the 1.0 release. Regenerating them makes the suite agree with the current engine instead of with the release it is meant to protect.</reason>
    </file>
  </locked_files>
  <contextual_instructions>
...
<rule>Elements in <temporary_workarounds> are short-lived stubs or hotfixes that must be refactored or deleted before their designated expiration date.</rule>
  <generated_elements>
    <element path="com.example.service.EvidenceBasedShowcase">
      <from>src/main/resources/openapi/checkout.yaml</from>
      <edit-instead>src/main/resources/openapi/checkout.yaml</edit-instead>
      <regenerate-with>mvn generate-sources</regenerate-with>
    </element>
  </generated_elements>
...
<rule>Elements in <generated_elements> are machine-generated: read them to understand behavior, but never edit them — changes are silently overwritten. Apply the change to the named source and regenerate instead.</rule>
  <load_bearing_elements>
    <element path="com.example.service.EvidenceBasedShowcase.settledOrderIds">
      <invariant>Settled orders stay in the list until the reconciliation job drains it</invariant>
      <breaks-if>Clearing eagerly drops in-flight settlements and silently under-reports revenue</breaks-if>
      <suppress-audit>true</suppress-audit>
    </element>
  </load_bearing_elements>
...
<rule>Elements in <load_bearing_elements> look redundant or over-defensive but are deliberate. You may refactor them, but the stated invariant must survive; do not "clean up" the oddity itself.</rule>
  <banned_apis>
    <element path="com.example.service.EvidenceBasedShowcase.totalWithTax(java.math.BigDecimal,java.math.BigDecimal)">
      <forbidden>java.lang.System.out, java.util.Date, java.lang.Double</forbidden>
      <use-instead>the injected org.slf4j.Logger, java.time.Instant, and java.math.BigDecimal</use-instead>
      <reason>Console output bypasses structured logging; Date and Double are unsafe for money and time</reason>
    </element>
  </banned_apis>
...
<rule>The APIs listed in <banned_apis> compile at those elements but are prohibited there. Use the stated replacement; if none is given, ask rather than substituting an equivalent.</rule>
  <thread_affinity_elements>
    <element path="com.example.service.EvidenceBasedShowcase.refreshCartBadge()">
      <affinity>the checkout-ui thread only</affinity>
      <marshal-via>CheckoutDispatcher.runOnUiThread</marshal-via>
      <symptom-if-violated>Cart totals render stale under load; no exception is thrown</symptom-if-violated>
    </element>
  </thread_affinity_elements>
...
<rule>Elements in <thread_affinity_elements> are safe on exactly one thread — the opposite of thread-safe. Never add locks or synchronization to "make them safe"; marshal the call onto the required thread instead.</rule>
  <mirrored_elements>
    <element path="com.example.service.EvidenceBasedShowcase.CATALOG_VERSION">
      <mirror>pom.xml:&lt;version&gt;</mirror>
      <mirror>README.md version badge</mirror>
      <mirror>docs/CHANGELOG.md</mirror>
      <reason>The release version is duplicated across build config, docs, and the badge</reason>
      <enforced-by>ProjectFactsConsistencyTest</enforced-by>
    </element>
  </mirrored_elements>
