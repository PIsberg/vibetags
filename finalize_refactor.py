...
path = r'c:\dev\private\vibetags\vibetags\src\main\java\se\deversity\vibetags\processor\AIGuardrailProcessor.java'

with io.open(path, mode='r', encoding='utf-8') as f:
    content = f.read()

# Target block to replace (the legacy body inside generateFiles)
target_start = '        if (root == null) return; // defensive'
target_end = '        return false; // do not claim annotations — other processors must still see them'

start_idx = content.find(target_start)
end_idx = content.find(target_end, start_idx) + len(target_end)

if start_idx == -1 or end_idx == -1:
    print(f"Error: Block not found. start_idx={start_idx}, end_idx={end_idx}")
    exit(1)

# New generateFiles body
replacement = r"""        if (log != null) {
...
modified_content = content[:start_idx] + replacement + content[end_idx:]
