...
Standalone benchmark harness for the `AIGuardrailProcessor`. Nine test categories live here:
...
| TESTING.md routing | `TestingMdRoutingStressTest` (since 1.3.6) | What routing a test round's guardrails out of the always-loaded file saves, on sources written to disk so a source set can be resolved at all |
| Platform breadth | `PlatformBreadthStressTest` (since 1.3.7) | Allocation, wall-clock, generated bytes and always-loaded bytes as the number of opted-in outputs grows from 1 to all 77. The volume sweeps opt into six of them, so this is the only place the other 50-odd are measured |
| Incremental rebuild | `IncrementalRebuildStressTest` (since 1.3.7) | What the *second* build of unchanged sources costs. Every other test here measures a cold build, because each gives its N a fresh `@TempDir` and the write cache and build fingerprint are therefore unreachable |
...
### A JMH run that measured nothing still exits 0

`java -jar target/benchmarks.jar <Class> -rf json -rff out.json` exits **0** when every benchmark
in the class fails, having written `[]`. There is no other signal: the stack traces scroll past in
the middle of the run and the status is clean.

`ProcessorHotPathBenchmark` was dead this way from 1.3.5 to 1.3.7. Its `@Setup(Level.Trial)`
created every service path as a file, and #684 added `.windsurf/rules/+vibetags-safety.md` inside
the `.windsurf/rules` directory service, so whichever came first in map order won and creating the
other threw `FileAlreadyExistsException`. Trial setup, so all six benchmarks failed. CI does not
run JMH, and the empty file is only obviously empty if you open it.

**Check the file, not the exit code:**

```bash
python -c "import json,sys; d=json.load(open(sys.argv[1])); print(len(d), 'benchmarks')" \
    results/$TAG/jmh.json     # must print 6
```
...
The volume sweeps are blind in three more ways, each now covered by its own class rather than left
implicit:

- **Six opt-in files out of 65.** Their fixture has created the same six since 0.5.4, so their
  overhead column is a six-platform project's. A fully opted-in project allocates about 4.2x that.
  See `PlatformBreadthStressTest`.
- **Cold builds only.** Each N gets a fresh `@TempDir`, so the write cache and the build
  ...
  That test reports its saving against two denominators. Against `-proc:none` a no-op rebuild
  saved 3.7 % at N=1000, which is where #834's headline came from. Against a no-op *processor*,
  which takes javac's own annotation-processing subsystem out of the base, the same saving was
  14.7 % before #833 and is about zero from N=500 up since. #833 removed a sidecar re-parse that
  sat on the path the short-circuit skips, so the cold build got 9 MB cheaper and the warm one did
  not move. The short-circuit still saves the writes and about 15 % of wall-clock at N=1000.
  The remaining opportunity is `WarmOwn`, about 48 MB at N=1000, not the 225 MB the diluted column
  shows still standing. `HashOwn` is what an exit ahead of the collection walk would at least
